#!/usr/bin/env python3
"""
LoRA Fine-tuning for Nahawi - Hamza Augmentation Edition.
Rank 64 LoRA on V15 base with hamza-focused training data.
Target: Push from 81% → 90% by fixing hamza errors.
"""
...
class LoRAConfig:
    ...
    # LoRA settings - Bump to rank 64 for more capacity
    lora_rank: int = 64
    lora_alpha: int = 128  # Scaling factor (2x rank)
    lora_dropout: float = 0.05

    # Training - Use hamza augmentation data
    train_data: str = "/home/ubuntu/nahawi/data/lora_train_hamza_aug.json"
    output_dir: str = "/home/ubuntu/nahawi/lora_model_hamza"
    batch_size: int = 192  # Same as before
...
class NahawiGECWithLoRA(nn.Module):
    ...
    def add_lora_adapters(self):
        ...
        print(f"Trainable params: {trainable_params:,} ({100*trainable_params/total_params:.2f}%)", flush=True)
...
class LoRATrainer:
    def __init__(self, config: LoRAConfig):
        ...
        # Create model
        print("Creating model with LoRA...", flush=True)
    ...
    def train(self):
        print("\n" + "=" * 70, flush=True)
        print("LORA HAMZA TRAINING - RANK 64", flush=True)
        print("=" * 70, flush=True)
        print(f"Epochs: {self.config.num_epochs}", flush=True)
        print(f"Batch size: {self.config.batch_size}", flush=True)
        print(f"Gradient accumulation: {self.config.gradient_accumulation}", flush=True)
        print(f"Effective batch: {self.config.batch_size * self.config.gradient_accumulation}", flush=True)
        print(f"Learning rate: {self.config.learning_rate}", flush=True)
        print(f"LoRA rank: {self.config.lora_rank}", flush=True)
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                if (i + 1) % self.config.gradient_accumulation == 0:
                    ...
                    if global_step % 100 == 0:
                        ...
                        print(f"Epoch {epoch+1} | Step {global_step} | Loss: {avg_loss:.4f} | LR: {lr:.2e}", flush=True)
            ...
            # End of epoch
            avg_loss = epoch_loss / num_batches
            print(f"\nEpoch {epoch+1} complete | Avg Loss: {avg_loss:.4f}", flush=True)
